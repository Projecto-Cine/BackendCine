...
public class PurchaseServiceImpl implements PurchaseService {
    ...
    public PurchaseResponseDTO create(PurchaseRequestDTO dto) {
        ...
        if (!screening.getStartDatetime().isAfter(LocalDateTime.now())) {
            throw new ScreeningAlreadyPassedException("This screening has already passed");
        }
        ...
        if (hasChild && !hasAdult) {
            throw new MinorWithoutAdultException("A minor must be accompanied by at least one adult in the same purchase");
        }
        ...
        for (TicketRequestDTO ticketRequest : ticketRequests) {
            Seat seat = seatRepository.findById(ticketRequest.seatId())
                    .orElseThrow(() -> new ResourceNotFoundException("Seat not found with id: " + ticketRequest.seatId()));
            ...
            if (screeningSeat.isOccupied()) {
                throw new SeatAlreadyTakenException("Seat " + seat.getRow() + seat.getNumber() + " is already taken");
            }
        ...
        BigDecimal discountAmount = PriceCalculator.applyFidelityDiscount(adultSubtotal, user.getYearlyVisits(), allTypes);
    ...
    public PurchaseResponseDTO confirm(Long purchaseId) {
        ...
        user.setYearlyVisits(user.getYearlyVisits() + 1);
    ...
    public PurchaseResponseDTO cancel(Long purchaseId) {
        ...
        if (!purchase.getScreening().getStartDatetime().isAfter(LocalDateTime.now())) {
            throw new ScreeningAlreadyPassedException("Cannot cancel a purchase for a screening that has already passed");
        }
    ...
    private void validateAgeRating(User user, Movie movie) {
        ...
        if (userAge < minAge) {
            throw new AgeRestrictionException(
                    "User does not meet the minimum age requirement of " + minAge + " years for this movie");
        }
    }
}
